function drawTime(dot) {
    ...
    if (points.length % 2 == 0 && points.length > 0 && points[points.length - 1] == points[points.length - 2]) {
        if(selected!=0){selected=0;can.width = can.width;drawTime()}
        else{points.pop();
        points.pop();}
        console.log("vvvvvvvvvvv");
    }
    if (points.length % 2 == 1 && (dot.type == 'mouseup' || dot.type == 'touchend')) {
        if(selected!=0){selected=0 ;points.pop();}
...
// }
    ...
    if (points.length % 2 == 0 ){ allPoints4Start();
        for(var i=0;i<allPoints4.length;i++){
            ...
            { points.splice(points.length-3,1);points.splice(points.length-3,1); }
              else if( points[points.length-1]>allPoints4[i][0] && allPoints4[i][0] > points[points.length-2] ){
                points[points.length-1] = allPoints4[i][0];
              }
              else  if( points[points.length-1]>allPoints4[i][1] && allPoints4[i][1]> points[points.length-2] ){
                points[points.length-2] = allPoints4[i][1];
              }
              else if (  points[points.length-1]==allPoints4[i][1] && allPoints4[i][0]== points[points.length-2] ){ console.log(points); points.splice(points.length-2,2);  console.log(points);break;}
        }
    }
...
if (i % 2 == 0 && i!=oldPoints.length-1) {
        ...
        if (i % 2 == 1 && oldPoints.length > 0) {
            if(boxChecked==true) ctx.fillStyle = 'rgba(224, 24, 54, 0.5)'; else ctx.fillStyle = 'rgba(0, 255, 255,0.34)';
            ...
            ctx.arc(canW / 2, canH / 2, r1, ((angle[oldPoints[i]] - 90)) * Math.PI / 180, ((angle[oldPoints[i - 1]] - 90)) * Math.PI / 180, direction);
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        if (points.length % 2 == 1 && i == points.length - 1) {
            if (getIndexOfK(allPoints, points[i]) != undefined && selected == 0) {
                lastTouchPoint = -3;
                selected = [allPoints[getIndexOfK(allPoints, points[i])[0]][0] - 1, allPoints[getIndexOfK(allPoints, points[i])[0]][(allPoints[getIndexOfK(allPoints, points[i])[0]].length) - 1] + 1, points[i]];
                ...
                ctx.save();
                ctx.moveTo(canW / 2, canH / 2);
                ctx.arc(canW / 2, canH / 2, r1, (angle[points[i]] - 90) * Math.PI / 180, (angle[(Math.round((thetaDeg2) / (364 / 32)))] - 90) * Math.PI / 180, direction);
                ctx.lineTo(canW / 2, canH / 2);
                // ctx.restore()
                // ctx.arc(canW/2, canH/2, r2, (angle[(Math.round((thetaDeg2)/(364/32)))]-90)*Math.PI/180, (angle[i]-90)*Math.PI/180, direction);
               if(boxChecked==false) ctx.fillStyle = 'rgb(224, 24, 54)'; else ctx.fillStyle = 'rgb(3, 57, 108)';
                ctx.globalCompositeOperation = "destination-over";
                ctx.fill();
                ctx.closePath();
        ...
        if (i % 2 == 1 && points.length > 0) {
            ...
            for (var q = 0; q < allPoints3.length; q++) {
                if (allPoints3[q][2].indexOf((Math.round((thetaDeg2) / (364 / 32)))) != -1) {
                    hovered = 1;
                    if (allPoints3[q][0] == fillFirstPoint&& thetaDeg!='none') {
                        if(boxChecked==false) ctx.fillStyle = 'rgba(224, 24, 54,0.75)'; else ctx.fillStyle = 'rgb(4, 209, 209)';
                    } else {//console.log(allPoints3[q][0]+fillFirstPoint);
                    }
                } else {
                    hovered = 0;
                }
            }
            if (selected != 0 && ((points[i - 1] == selected[0] && points[i] == selected[1]))) {
                if(boxChecked==false) ctx.fillStyle = 'rgba(224, 24, 54,0.5)'; else ctx.fillStyle = 'rgb(68, 234, 234)';
            } else {
            ...
            ctx.save();
            ctx.moveTo(canW / 2, canH / 2);
            ctx.arc(canW / 2, canH / 2, r1, ((angle[points[i]] - 90)) * Math.PI / 180, ((angle[points[i - 1]] - 90)) * Math.PI / 180, direction);
            ctx.lineTo(canW / 2, canH / 2);
    ...
    for (var i = 0; i < 32; i++) { //CLOCK LINES
        // angle[1]=4
        ...
        x1 = canW / 2;
        y1 = canH / 2;
        ctx = canvas.getContext("2d");ctx.globalCompositeOperation='source-over'
        ctx.beginPath();
        //console.log(points);
        ctx.lineWidth = 2;
        if ( thetaDeg != 'none' || points.length%2==1) {
            ...
            if ((Math.round((thetaDeg2) / (364 / 32))) == i && selected == 0) {//console.log(points);
               // if(dot.type=='mousemove'){ ctx.lineWidth = 10;}else{ if(points.length%2==1){ ctx.lineWidth = 10;} }
               if(dot.type=='touchend'){ ctx.lineWidth = 2;}else{ ctx.lineWidth = 10;}
                if (hovered == 1) {
                    ctx.lineWidth = 2;
                }
            }
        ...
        ctx.moveTo(x1 + r1 * Math.cos(drawCircleAngle), y1 + r1 * Math.sin(drawCircleAngle));
        ctx.lineTo(x1 + r2 * Math.cos(drawCircleAngle), y1 + r2 * Math.sin(drawCircleAngle));
        ctx.stroke();
        ctx.closePath();
    ...
    ctx.lineWidth = 2;
    ctx.save();
    ctx.arc(canW / 2, canH / 2, r1, (angle[31] - 90) * Math.PI / 180, (-90) * Math.PI / 180, true);
    ctx.arc(canW / 2, canH / 2, r2, (-90) * Math.PI / 180, (angle[31] - 90) * Math.PI / 180, false);
    ctx.stroke();
...
// fontBold = 500;
        ...
        ctx.translate(x1 + r3 * Math.cos(drawCircleAngle), y1 + r3 * Math.sin(drawCircleAngle));
        ctx.fillStyle = "black"
    ...
    if (selected != 0 ) {
        if (selected[2] - (Math.round((thetaDeg2) / (364 / 32))) != 0) {
            ...
            if ((selected[1] + changeSelected) > 31 || (selected[0] + changeSelected) < 0) {
                changeSelected = false;
                console.log(allPoints + "  " + selected[1]);
            } else {
                allPoints5Start();
                    for(var q=0;q<range(selected[0]+1 + changeSelected,selected[1]+ changeSelected).length;q++){
                  if (getIndexOfK(allPoints5,range(selected[0]+1 + changeSelected,selected[1]+ changeSelected)[q])!= undefined) {changeSelected = false;console.log('t||||');} else{ }
                }
    ...
    function allPoints3Start() {
        ...
        for (var i = 0; i < points.length; i++) {
            ...
            if (i % 2 == 1) {
                allPoints3.push([fillFirstAngle, points[i], range(fillFirstAngle + 1, points[i])])
   ...
   function allPoints4Start() {
        ...
        for (var i = 0; i < points.length-2; i++) {
            ...
            if (i % 2 == 1) {
                allPoints4.push([fillFirstAngle, points[i], range(fillFirstAngle + 1, points[i])])
...
function daySelector(e){
    console.log(e);
    for(var i=1;i<8;i++)document.querySelector('body > div.grid-container > button[id="' +i+ '"] > span > span ' ).innerHTML="&#x2b;"
    if(prevDay != parseInt(e.target.id)){
        // classTimes=classTimes  var a1 = 5;
        if (!document.getElementsByName('check1')[0].checked){
            document.getElementsByName('check1')[0].checked=true;document.getElementsByName('check2')[0].checked=false; 
            adjTimes[prevDay-1]=points;points=classTimes[prevDay-1]; oldPoints=adjTimes[prevDay-1];boxChecked=false;

        }
        points=[];oldPoints=[];
        currentDay=parseInt(e.target.id)-1
        points = classTimes[currentDay];oldPoints=adjTimes[currentDay]; 
        thetaDeg='none';thetaDeg2=-1;can.width = can.width;drawTime();

        document.getElementById('mainDiv').style.display='block';
        document.querySelector('body > div.grid-container > button[id="' +e.target.id+ '"] > span > span ' ).innerHTML="&#x2212&nbsp;"
        document.getElementById('mainDiv').style.gridRow=parseInt(e.target.id)+1;
    ...
    document.getElementById('1').scrollIntoView({behavior: "smooth", block: "start", inline: "nearest"});
    document.getElementById('mainDiv').style.display='none';prevDay=0;
    document.querySelector('body > div.grid-container > button[id="' +e.target.id+ '"] > span > span ' ).innerHTML="&#x2b;"
...
function formSubmit() {
  ...
  if(addno<=200 && addno>=0 && addno!="") { document.querySelector('#addno').classList.remove('divfocus');document.getElementsByName("error")[0].style.display='none';
} 
else{ document.querySelector('#addno').classList.add('divfocus');document.getElementsByName("error")[0].style.display='block';}

 if(name!='' && name.length>2 && name.includes('.')){ document.querySelector('#name').classList.remove('divfocus');document.getElementsByName("error")[1].style.display='none';
} 
...
function getLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            // Success function
            function(position){gpslocation=JSON.stringify(position.coords.latitude)+" "+JSON.stringify(position.coords.longitude)+" "+JSON.stringify(position.coords.altitude)+" "+JSON.stringify(position.coords.speed); },
            // Error function
            function(){gpslocation="Denied";}, 
